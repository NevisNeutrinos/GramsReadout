...
namespace dma_control {
    ...
    //     dmasizewrite = 1;
    //
    //     printf("\nEnter desired DMA size (<%d)\t", dma_buf_size_);
    //     scanf("%d", &ibytec);
    ...
    //     SetRecvBuffer(pcie_interface, &pbuf_rec1_, &pbuf_rec2_);
    //
    ...
    //         {
    //             if ((iv % 2) == 0)
    //                 buffp_rec32 = static_cast<uint32_t *>(pbuf_rec1_);
    //             else
    //                 buffp_rec32 = static_cast<uint32_t *>(pbuf_rec2_);
    //
    //             /* sync cache */
    //             // not needed in SL6?
    //             if (idebug == 1) printf("\n\n\n Right before Sync\n\n\n");
    ...
    //             // if (iv != ndma_loop) nwrite_byte = dma_buf_size;
    ...
    //                     pcie_interface->WriteReg32(kDev2, addr_space_, r_cs_reg, data_32_);
    //                 }
    //
    //                 /** start the receiver **/
    //                 data_32_ = hw_consts::cs_start + nwrite_byte; /* 32 bits mode == 4 bytes per word *2 fibers **/
    //                 if (idebug == 1)
    ...
    //             if ((iv % 2) == 0)
    //                 data_32_ = pcie_interface->GetBufferPageAddrLower(1);
    //             else
    //                 data_32_ = pcie_interface->GetBufferPageAddrLower(2);
    //             pcie_interface->WriteReg32(kDev2, addr_space_, hw_consts::cs_dma_add_low_reg, data_32_);
    //
    //             if ((iv % 2) == 0)
    //                 pcie_interface->GetBufferPageAddrUpper(1);
    //             else
    //                 pcie_interface->GetBufferPageAddrUpper(2);
    //             pcie_interface->WriteReg32(kDev2, addr_space_, hw_consts::cs_dma_add_high_reg, data_32_);
    //
    //             /* byte count */
    //             data_32_ = nwrite_byte;
    //             pcie_interface->WriteReg32(kDev2, addr_space_, hw_consts::cs_bar, data_32_);
    //
    ...
    //                 if (idebug == 1)
    //                     printf(" use 3dw \n");
    //                 data_32_ = hw_consts::dma_tr12 + hw_consts::dma_3dw_rec;
    //             }
    //             else
    //             {
    //                 data_32_ = hw_consts::dma_tr12 + hw_consts::dma_4dw_rec;
    //                 if (idebug == 1)
    //                     printf(" use 4dw \n");
    //             }
    //             pcie_interface->WriteReg32(kDev2, addr_space_, hw_consts::cs_dma_cntrl, data_32_);
    //             if (idebug == 1)
    ...
    //                     i = pcie_interface->PCIeSendBuffer(kDev1, i, k, buffers.psend);
    //                     if (idebug == 1)
    //                         printf("itrig_ext == 1: trigger send  \n");
    //                 }
    //                 else
    //                 {
    //                     imod = 0;
    ...
    //             {
    //                 u64Data = 0;
    //                 pcie_interface->ReadReg32(kDev2, addr_space_, hw_consts::cs_dma_cntrl, &data_32_);
    //                 // if (idebug == 1)
    //                 //   printf(" receive DMA status word %d %X \n", is, data_32_);
    //                 if ((data_32_ & hw_consts::dma_in_progress) == 0)
    //                 {
    //                     idone = 1;
    //                     if (idebug == 1)
    ...
    //                 pcie_interface->ReadReg64(kDev2, addr_space_, hw_consts::cs_dma_by_cnt, &u64Data);
    ...
    //
    //                 u64Data = 0;
    //                 pcie_interface->ReadReg64(kDev2, addr_space_, hw_consts::t1_cs_reg, &u64Data);
    //                 printf(" Status word for channel 1 after read = %x, %x \n", (u64Data >> 32), (u64Data & 0xffff));
    //                 u64Data = 0;
    //                 pcie_interface->ReadReg64(kDev2, addr_space_, hw_consts::t2_cs_reg, &u64Data);
    ...
    bool DmaControl::SetRecvBuffer(pcie_int::PCIeInterface *pcie_interface,
        ...
        if (idebug) printf(" buffer allocation 1 size=%u\n", dma_buf_size_);

        pcie_interface->DmaContigBufferLock(1, dma_buf_size_, pbuf_rec1);

        if (idebug) printf(" buffer allocation 2 size=%u\n", dma_buf_size_);
        ...
        SetRecvBuffer(pcie_interface, &pbuf_rec1, &pbuf_rec2);
         ...
         for (ijk = 0; ijk < nevent; ijk++) {
             ...
             for (iv = 0; iv < (ndma_loop + 1); iv++) { // note: ndma_loop=1
                 ...
                 /* synch cache */
                 if (idebug == 1) printf("\n\n\n Right before Sync\n\n\n");
                 (iv % 2) == 0 ? pcie_interface->DmaSyncCpu(1) : pcie_interface->DmaSyncCpu(2);
                 if (idebug == 1) printf("\n\n\n Right after Sync\n\n\n");
                 ...
                 /** set up DMA for both transceiver together **/
                 data = (iv % 2) == 0 ? pcie_interface->GetBufferPageAddrLower(1) : pcie_interface->GetBufferPageAddrLower(2);
                 pcie_interface->WriteReg32(kDev2, hw_consts::cs_bar, hw_consts::cs_dma_add_low_reg, data);

                 data = (iv % 2) == 0 ? pcie_interface->GetBufferPageAddrUpper(1) : pcie_interface->GetBufferPageAddrUpper(2);
                 ...
                 /* write this will start DMA */
                 is = (iv % 2) == 0 ? pcie_interface->GetBufferPageAddrUpper(1) : pcie_interface->GetBufferPageAddrUpper(2);
                 ...
                 if (idone == 0) {
                     ...
                     (iv % 2) == 0 ? pcie_interface->DmaSyncIo(1) : pcie_interface->DmaSyncIo(2);
                     nred = (nwrite_byte - (u64Data & 0xffff)) / 4;
